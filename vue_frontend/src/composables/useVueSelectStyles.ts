import { computed } from 'vue';

export function useVueSelectStyles() {
  return computed(() => ({
    // '--vs-min-height': '4rem',
    '--vs-padding': '0.5rem 0.5rem 0 0.5rem ',
    '--vs-background-color': 'var(--color-slate-50)',
    '--vs-disabled-background-color': 'var(--color-bg-lo)',
    '--vs-text-color': 'var(--color-slate-800)',
    '--vs-border': 'none',
    '--vs-border-radius': 'var(--radius-md, 0.375rem)',
    '--vs-font-weight': '400',
    '--vs-font-size': '.95rem',
    '--vs-font-family': 'var(--font-primary)',  
    '--vs-outline-width': '0',
    '--vs-outline-color': 'var(--color-slate-500)',
    '--vs-menu-z-index': '2000',
    '--vs-option-selected-background-color': 'var(--color-blue-400)',
    '--vs-option-disabled-background-color': 'var(--color-zinc-200)',
    '--vs-option-focused-text-color': 'var(--color-slate-800)',
    '--vs-option-focused-background-color': 'var(--vs-option-hover-background-color)',
    '--vs-option-hover-background-color': 'var(--color-blue-100)',
    '--vs-option-background-color': 'var(--color-white)',
    '--vs-option-text-color': 'var(--color-slate-800)',
    '--vs-option-font-size:': '.95rem',
    '--vs-option-selected-text-color': 'var(--color-slate-800)',
    '--vs-option-hover-text-color': 'var(--color-slate-800)',
    '--vs-indicator-icon-color': 'var(--color-slate-800)',
  }));
}


export const selectErrorStyle = (hasError: boolean) =>
  hasError
    ? {
        '--vs-border': '1px solid var(--color-orange-500)',
        '--vs-outline-color': 'var(--color-orange-500)',
      }
    : {};